...
describe('usersController (toutes méthodes)', () => {
  ...
  describe('login', () => {
    ...
    it('réussit à connecter et génère un token', async () => {
      ...
      const fakeUser = { id: 8, role: 'client', password: 'hash' };
  ...
  describe('getAll', () => {
    it('renvoie la liste des users', async () => {
      ...
      prisma.user.findMany.mockResolvedValue(fakeList);
  ...
  describe('update', () => {
    ...
    it('met à jour user sans password', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ message: 'Utilisateur mis à jour.', user: fakeUser });
    });
  });
